...
public class GameLogic {
    ...
    private AudioManager audioManager; // 新增：音频管理器
    ...
    private void handleGameStateSounds() {
        ...
        if (!wasPlaying && gameState == GameState.PLAYING) {
            // 从开始界面进入游戏
            audioManager.playSound(AudioManager.SoundType.START_GAME);
            audioManager.playBackgroundMusic();
        } else if (gameState == GameState.VICTORY && wasPlaying) {
            // 游戏胜利
            audioManager.stopBackgroundMusic();
            audioManager.playSound(AudioManager.SoundType.VICTORY);
        } else if (gameState == GameState.GAME_OVER && wasPlaying) {
            // 游戏失败
            audioManager.stopBackgroundMusic();
            audioManager.playSound(AudioManager.SoundType.GAME_OVER);
    ...
    private void updatePlayingState() {
        ...
        if (moveSoundCooldown <= 0 && (input.upPressed || input.downPressed || input.leftPressed || input.rightPressed)) {
            audioManager.playSound(AudioManager.SoundType.MOVE);
    ...
    private void handleCollisions() {
        ...
        for (Bullet bullet : bullets) {
            ...
            if (bullet.collidesWith(player.x, player.y, GameConfig.TANK_WIDTH)) {
                bullet.setAlive(false);
                player.lives--;
                audioManager.playSound(AudioManager.SoundType.HIT);
                if (player.lives <= 0) {
                    player.lives = 0;
                    player.dead = true;
                }
